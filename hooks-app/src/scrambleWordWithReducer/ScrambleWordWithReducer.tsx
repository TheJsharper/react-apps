...
import { getInitialScrambleWordsState, scrambleWordReducer } from './reducer/scrambleReducer';


export const ScrambleWordsWithReducer = () => {

    const [state, dispatch] = useReducer(scrambleWordReducer, getInitialScrambleWordsState());


    const { currentWord, errorCounter, guess, isGameOver, maxAllowErrors, maxSkips, points, scrambleWord, skipCounter, words, totalLength } = state;
    ...
    const handleSkip = () => dispatch({ type: 'SKIP_WORD' });



    const handlePlayAgain = () => dispatch({ type: 'START_NEW_GAME', paayload: getInitialScrambleWordsState() })
