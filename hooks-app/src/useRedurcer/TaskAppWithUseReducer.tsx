...
export const TasksAppWithUseReducer = () => {

  const [inputValue, setInputValue] = useState('');

  const [state, distpach] = useReducer(taskReducer, getTasksInitialState())

  const { todos, completed: completedCount, length: totalCount } = state;


  useEffect(()=> localStorage.setItem('tasks-state', JSON.stringify(state)), [state]);

  const addTodo = () => {
    if (inputValue.length <= 0) return;

    distpach({ type: 'ADD_TODO', payload: inputValue });

    setInputValue('');

  };

  const toggleTodo = (id: number) => distpach({ type: 'TOGGLE_TODO', payload: id });

  const deleteTodo = (id: number) => distpach({ type: 'DELETE_TODO', payload: id });
      ...
      <div className="mx-auto max-w-2xl">
        <div className="mb-8 text-center">
          ...
          <p className="text-slate-600">
            Keep up your tasks organized and try to do them
          </p>
        </div>
          ...
          <CardContent>
            {todos.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-slate-500 text-lg mb-2">Nothing to do...</p>
